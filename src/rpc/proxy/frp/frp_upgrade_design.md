...
10. release TCP relay 连接，停止 relay 接收（relay 异步读端点的 error 完成被 `p2p_success_` 标志静默拦截，不会误触发断开回调）
11. `proxy_data_channel` 底层切换完成，触发 `on_p2p_upgraded` 回调，同时启动 UDP keepalive 定时器（每 10s 发送 1 字节保活包，防止 NAT 映射超时）
...
- 轮间隔：`1s`，计时器驱动，双方独立
- 最大轮次：`32`
- `p2p` 会话 keepalive 周期：`10s`（1 字节 UDP 保活包）
