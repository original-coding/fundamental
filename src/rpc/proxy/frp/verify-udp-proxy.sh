...
WORK_DIR="$(mktemp -d /tmp/frp-verify-udp-XXXXXX)"
...
{ "threads": 2, "listen_tcp_port": ${SERVER_TCP_PORT}, "listen_udp_port": 0,
  "traffic_secret": "traffic-secret-demo", "allowed_register_keys": ["k1"],
  "ssl": { "disable_ssl": true } }
EOF

# Provider config: UDP service
cat > "${WORK_DIR}/config/provider.json" <<EOF
{ "threads": 2, "public_server_host": "127.0.0.1",
  "public_server_tcp_port": ${SERVER_TCP_PORT}, "public_server_udp_port": 0,
  "traffic_secret": "traffic-secret-demo", "nat_type": 0,
  "ssl": { "disable_ssl": true },
  "groups": [
    {
      "register_key": "k1",
      "services": [ { "service_name": "echo-udp", "service_type": 1,
                  "target_host": "127.0.0.1", "target_port": ${BACKEND_PORT} } ]
    }
  ]
}
EOF

# Accessor config: UDP listener
...
{ "threads": 2, "public_server_host": "127.0.0.1",
  ...
  "groups": [
    {
      "register_key": "k1",
      "listeners": [ { "service_name": "echo-udp", "service_type": 1,
                   "listen_host": "127.0.0.1", "listen_port": ${ACCESSOR_PORT} } ]
    }
  ]
}
EOF
...
echo "========================================"
echo "  FRP UDP Proxy Local Verification"
echo "========================================"
...
if grep -q '\[TEST PASSED\]' "${WORK_DIR}/echo_client.log"; then
    echo "✅ PASSED: UDP proxy relay works"
    echo ""
    echo "=== tail logs ==="
    ...
    echo "❌ FAILED: UDP proxy echo test did not pass"
    echo ""
