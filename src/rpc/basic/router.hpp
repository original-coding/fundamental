...
public:
    ...
    void register_handler(std::string const& name, Function f, bool pub = false) {
        uint32_t key = md5_hasher::MD5Hash32(name.data());
    ...
    void register_wrapper_handler(
        std::string const& name,
        const std::function<void(std::weak_ptr<connection>, std::string_view, std::string&)>& f) {
        uint32_t key = md5_hasher::MD5Hash32(name.data());
        if (key2func_name_.find(key) != key2func_name_.end()) {
            throw std::invalid_argument("duplicate registration key !");
        } else {
            if (!f) {
                throw std::invalid_argument("invalid wrapper handler!");
            }
            key2func_name_.emplace(key, name);
            map_invokers_[key] = f;
        }
    ...
    template <typename T>
    route_result_t route(uint32_t key, std::string_view data, std::weak_ptr<T> conn) {
        ...
        try {
            ...
            result = codec.pack_args_str(
                result_code::FAIL, std::string("unknown exception occur when call ").append(get_name_by_key(key)));
            route_result.ec = router_error::no_such_function;
        }
...
private:
    ...
    std::unordered_map<uint32_t, std::function<void(std::weak_ptr<connection>, std::string_view, std::string&)>>
        map_invokers_;
